...
// Respond to POLL
void ClientManager::respond_Poll(int my_index, int sd){

	buf="";//buf 초기화
	// 등록시 sd를 CSession[index]에 등록 및 number++
	...
	switch(int k=Parser(buf, this)) {
		...
		case 1: {	//id 중복체크 후 중복이 아니면 CSession[index]에 ID 등록
							...
							cout<<get_registration_ID(buf)<<endl;
							cout<<CSession[my_index]->get_myID()<<endl;
